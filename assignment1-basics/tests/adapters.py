...
from cs336_basics.tokenizer import train_bpe_tokenizer ,  Tokenizer
from cs336_basics.model import Linear, Embedding, RMSNorm, SwiGLU , softmax , RoPE, Attention, Multi_Head_Attention, TransformerBlock, TransformerLM
...
def run_multihead_self_attention(
    ...
    """
    mhn = Multi_Head_Attention(d_model , num_heads)
    mhn.q_proj.weight = nn.Parameter(q_proj_weight)
    mhn.k_proj.weight = nn.Parameter(k_proj_weight)
    mhn.v_proj.weight = nn.Parameter(v_proj_weight)
    mhn.output_proj.weight = nn.Parameter(o_proj_weight)
    return mhn(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    mhn = Multi_Head_Attention(d_model , num_heads, max_seq_len , theta)
    mhn.q_proj.weight = nn.Parameter(q_proj_weight)
    mhn.k_proj.weight = nn.Parameter(k_proj_weight)
    mhn.v_proj.weight = nn.Parameter(v_proj_weight)
    mhn.output_proj.weight = nn.Parameter(o_proj_weight)
    return mhn(in_features , token_positions = token_positions)
...
def run_transformer_block(
    ...
    # we find the seq_len from the input features
    ...
    Args:
        ...
        weights (dict[str, Tensor]):
            ...
            - `layers.{num_layers}.attn.output_proj.weight`
                Weight of the multi-head self-attention output projection
                Shape is ((d_model / num_heads) * num_he ads, d_model).
